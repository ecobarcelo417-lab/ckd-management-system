              ...
              (err, rows) => {
                ...
                db.all(
                  `SELECT scheduled_date as date, COUNT(*) as count
                   ...
                   WHERE scheduled_date >= date(?, '-13 days') AND scheduled_date <= ?
                  ...
                  (err, trendRows) => {
                    ...
                    db.get(
                      `SELECT COUNT(*) as count FROM dialysis_sessions
                       WHERE status = 'completed' AND scheduled_date >= date(?, '-6 days') AND scheduled_date <= ?`,
                      [today, today],
                      (err, thisWeekRow) => {
                        db.get(
                          `SELECT COUNT(*) as count FROM dialysis_sessions
                           WHERE status = 'completed' AND scheduled_date >= date(?, '-13 days') AND scheduled_date <= date(?, '-7 days')`,
                          [today, today],
                          (err, lastWeekRow) => {
                            ...
                            db.get(
                              `SELECT AVG(kt_v) as avg FROM dialysis_sessions
                               WHERE kt_v IS NOT NULL AND scheduled_date >= date(?, '-29 days') AND scheduled_date <= ?`,
                              [today, today],
                              (err, ktvNowRow) => {
                                db.get(
                                  `SELECT AVG(kt_v) as avg FROM dialysis_sessions
                                   WHERE kt_v IS NOT NULL AND scheduled_date >= date(?, '-59 days') AND scheduled_date <= date(?, '-30 days')`,
                                  [today, today],
                                  (err, ktvPrevRow) => {
